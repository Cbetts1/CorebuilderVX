"""
hardware_menu.py — Tools Center Hardware Tools menu.

Loads tools from the registry for the 'hardware' group,
...
def show():
    """Display the Hardware Tools menu."""
    navigation.push("Hardware Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "hardware"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("hardware"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Detect Hardware", lambda: print("  Running: Detect Hardware")),
        ("{}", "Benchmark CPU", lambda: print("  Running: Benchmark CPU")),
    ]
