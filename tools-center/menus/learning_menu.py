"""
learning_menu.py — Tools Center Learning menu.

Loads tools from the registry for the 'learning' group,
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "learning"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("learning"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Start Lesson", lambda: print("  Running: Start Lesson")),
        ("{}", "List Lessons", lambda: print("  Running: List Lessons")),
    ]
