"""
storage_menu.py — Tools Center Storage Tools menu.

Loads tools from the registry for the 'storage' group,
...
def show():
    """Display the Storage Tools menu."""
    navigation.push("Storage Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "storage"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("storage"))
...
def _fallback_items():
    ...
    return [
        ("{}", "List Files", lambda: print("  Running: List Files")),
        ("{}", "Backup Files", lambda: print("  Running: Backup Files")),
    ]
