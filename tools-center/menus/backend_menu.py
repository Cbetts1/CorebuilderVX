"""
backend_menu.py — Tools Center Backend Tools menu.

Loads tools from the registry for the 'backend' group,
...
def show():
    """Display the Backend Tools menu."""
    navigation.push("Backend Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "backend"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("backend"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Start API Server", lambda: print("  Running: Start API Server")),
        ("{}", "Test Endpoint", lambda: print("  Running: Test Endpoint")),
    ]
