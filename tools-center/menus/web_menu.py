"""
web_menu.py — Tools Center Web Tools menu.

Loads tools from the registry for the 'web' group,
...
def show():
    """Display the Web Tools menu."""
    navigation.push("Web Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "web"]
    result = []
    for tool in tools:
