"""
simulation_menu.py — Tools Center Simulation Tools menu.

Loads tools from the registry for the 'simulation' group,
...
def show():
    """Display the Simulation Tools menu."""
    navigation.push("Simulation Tools")

    items = _build_items()
    menu_engine.show_menu(
        title="🎛️  Simulation Tools",
        items=items,
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "simulation"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("simulation"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Run Simulation", lambda: print("  Running: Run Simulation")),
        ("{}", "List Simulations", lambda: print("  Running: List Simulations")),
    ]
