"""
network_menu.py — Tools Center Network Tools menu.

Loads tools from the registry for the 'network' group,
...
def show():
    """Display the Network Tools menu."""
    navigation.push("Network Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "network"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("network"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Ping Host", lambda: print("  Running: Ping Host")),
        ("{}", "Scan Ports", lambda: print("  Running: Scan Ports")),
    ]
