"""
settings_menu.py — Tools Center Settings menu.

Loads tools from the registry for the 'settings' group,
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "settings"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("settings"))
...
def _fallback_items():
    ...
    return [
        ("{}", "View Settings", lambda: print("  Running: View Settings")),
        ("{}", "Edit a Setting", lambda: print("  Running: Edit a Setting")),
    ]
