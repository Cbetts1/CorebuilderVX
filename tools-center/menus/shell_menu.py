"""
shell_menu.py — Tools Center Shell Tools menu.

Loads tools from the registry for the 'shell' group,
...
def show():
    """Display the Shell Tools menu."""
    navigation.push("Shell Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "shell"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("shell"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Open Shell", lambda: print("  Running: Open Shell")),
        ("{}", "Run Shell Command", lambda: print("  Running: Run Shell Command")),
    ]
