"""
build_menu.py — Tools Center Build Tools menu.

Loads tools from the registry for the 'build' group,
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "build"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("build"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Compile Project", lambda: print("  Running: Compile Project")),
        ("{}", "Generate Makefile", lambda: print("  Running: Generate Makefile")),
    ]
