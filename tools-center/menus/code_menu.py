"""
code_menu.py — Tools Center Code Tools menu.

Loads tools from the registry for the 'code' group,
...
def show():
    """Display the Code Tools menu."""
    navigation.push("Code Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "code"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("code"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Run Code", lambda: print("  Running: Run Code")),
        ("{}", "Lint Code", lambda: print("  Running: Lint Code")),
        ("{}", "Format Code", lambda: print("  Running: Format Code")),
    ]
