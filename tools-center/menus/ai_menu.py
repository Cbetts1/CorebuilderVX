"""
ai_menu.py — Tools Center AI Tools menu.

Loads tools from the registry for the 'ai' group,
...
def show():
    """Display the AI Tools menu."""
    navigation.push("AI Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "ai"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("ai"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Run AI Model", lambda: print("  Running: Run AI Model")),
        ("{}", "List AI Models", lambda: print("  Running: List AI Models")),
    ]
