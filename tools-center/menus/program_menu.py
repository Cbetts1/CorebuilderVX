"""
program_menu.py — Tools Center Program Tools menu.

Loads tools from the registry for the 'program' group,
...
def show():
    """Display the Program Tools menu."""
    navigation.push("Program Tools")
...
def _build_items():
    ...
    tools = [t for t in state.registry if t.get("group") == "program"]
    result = []
    for tool in tools:
        icon  = tool.get("icon", icons.get("program"))
...
def _fallback_items():
    ...
    return [
        ("{}", "Install Package", lambda: print("  Running: Install Package")),
        ("{}", "List Packages", lambda: print("  Running: List Packages")),
    ]
